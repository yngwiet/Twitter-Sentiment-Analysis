...
def main():
    ...
    print "unigram + bigram model starts..."
    ...
    if os.path.isfile('./data/processed/unibigram/preptrainingdata.pickle') \
            and os.path.isfile('./data/processed/unibigram/preptestdata.pickle'):

        print "preptrainingdata and preptestdata detected, load files..."

        preptrainingdata_f = open('./data/processed/unibigram/preptrainingdata.pickle', 'r')
        preptrainingdata = pickle.load(preptrainingdata_f)

        preptestdata_f = open('./data/processed/unibigram/preptestdata.pickle', 'r')
        ...
        save_documents = open('./data/processed/unibigram/preptrainingdata.pickle', 'w')
        pickle.dump(preptrainingdata, save_documents)
        save_documents.close()

        # store preprocessed test data
        save_documents = open('./data/processed/unibigram/preptestdata.pickle', 'w')
    ...
    if os.path.isfile('./data/processed/unibigram/trainingfeaset.pickle') \
            and os.path.isfile('./data/processed/unibigram/testfeaset.pickle')\
            and os.path.isfile('./data/processed/unibigram/word_features.pickle'):

        print "trainingfeaset, testfeaset and word_features detected, load files..."

        trainingfeaset_f = open('./data/processed/unibigram/trainingfeaset.pickle', 'r')
        trainingfeaset = pickle.load(trainingfeaset_f)

        testfeaset_f = open('./data/processed/unibigram/testfeaset.pickle', 'r')
        testfeaset = pickle.load(testfeaset_f)

        word_features_f = open('./data/processed/unibigram/word_features.pickle', 'r')
        ...
        # set desired # of unigram features and bigram features in the second and third parameters
        word_features = fea_extractor.get_unibigram_features(all_words, 2000, 2000)

        print "generating word_features done..."

        del all_words  # release some memory

        trainingfeaset = [(fea_extractor.construct_unibigram_feaset(row[0], word_features), row[1]) for row in preptrainingdata]
        testfeaset = [(fea_extractor.construct_unibigram_feaset(row[0], word_features), row[1]) for row in preptestdata]

        print "generating trainingfeaset and testfeaset done... great progress!"

        save_documents = open('./data/processed/unibigram/word_features.pickle', 'w')
        pickle.dump(word_features, save_documents)
        save_documents.close()

        save_documents = open('./data/processed/unibigram/trainingfeaset.pickle', 'w')
        pickle.dump(trainingfeaset, save_documents)
        save_documents.close()

        save_documents = open('./data/processed/unibigram/testfeaset.pickle', 'w')
    ...
    if os.path.isfile('./data/processed/unibigram/NB_classifier.pickle'):
        NB_classifier_f = open("./data/processed/unibigram/NB_classifier.pickle", "r")
        ...
        save_classifier = open("./data/processed/unibigram/NB_classifier.pickle", "w")
    ...
    if os.path.isfile('./data/processed/unibigram/MaxEntClassifier.pickle'):
        MaxEntClassifier_f = open('./data/processed/unibigram/MaxEntClassifier.pickle','r')
        ...
        save_classifier = open("./data/processed/unibigram/MaxEntClassifier.pickle", "w")
    ...
    testset = fea_extractor.construct_unibigram_svm_feaset(preptestdata, word_features)

    if os.path.isfile('./data/processed/unibigram/svm_classifier.model'):

        svm_classifier = svm_load_model('./data/processed/unibigram/svm_classifier.model')

    else:

        trainingset = fea_extractor.construct_unibigram_svm_feaset(preptrainingdata, word_features)
        ...
        svm_save_model('./data/processed/unibigram/svm_classifier.model', svm_classifier)
